  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatsCard
          title={t('dashboard.repairs_count')}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('dashboard.weekly_profit')}</h3>
          <div className="h-80">
            {profitData.some(d => d.profit > 0) ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={profitData}>
                  ...
                  <XAxis dataKey="name" />
                  <YAxis />
                  <Tooltip formatter={(value) => [`${value} ${t('common.currency')}`, t('repairs.profit')]} />
              ...
              <div className="flex items-center justify-center h-full text-gray-500">
                <div className="text-center">
                  ...
                  <p>{t('dashboard.no_profit_data')}</p>
                  <p className="text-sm">{t('dashboard.add_repairs_message')}</p>
                </div>
              </div>
            )}
          </div>
        ...
        {/* Popular Models */}
        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('dashboard.popular_models')}</h3>
          <div className="h-80">
            {popularModels.length > 0 ? (
              ...
              <div className="flex items-center justify-center h-full text-gray-500">
                <div className="text-center">
                  ...
                  <p>{t('dashboard.no_models_data')}</p>
                  <p className="text-sm">{t('dashboard.add_repairs_for_models')}</p>
                </div>
              </div>
            )}
          </div>
      ...
      {/* Common Issues */}
      <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
        <h3 className="text-lg font-semibold text-gray-900 mb-4">{t('dashboard.common_issues')}</h3>
        {commonIssues.length > 0 ? (
          ...
          <div className="text-center py-8 text-gray-500">
            ...
            <p>{t('dashboard.no_issues_data')}</p>
            <p className="text-sm">{t('dashboard.add_repairs_for_issues')}</p>
          </div>
        )}
      </div>
    </div>
